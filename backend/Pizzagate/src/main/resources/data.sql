INSERT INTO pizzerias (name, address, phone, owner_subject)
SELECT 'Napoli Centro', 'Rua das Flores 10, Porto', '+351 220 100 100', 'owner-a'
WHERE NOT EXISTS (
  SELECT 1
  FROM pizzerias
...
INSERT INTO pizzerias (name, address, phone, owner_subject)
SELECT 'Roma Antica', 'Avenida da Liberdade 22, Lisboa', '+351 210 200 200', 'owner-b'
WHERE NOT EXISTS (
  SELECT 1
  FROM pizzerias
...
INSERT INTO pizzerias (name, address, phone, owner_subject)
SELECT 'Sicilia Slice', 'Praça do Mercado 5, Coimbra', '+351 239 300 300', 'owner-a'
WHERE NOT EXISTS (
  SELECT 1
  FROM pizzerias
...
INSERT INTO pizzeria_staff (pizzeria_id, staff_subject)
SELECT p.id, 'staff-a'
FROM pizzerias p
WHERE p.name = 'Napoli Centro'
  AND NOT EXISTS (
    SELECT 1
    FROM pizzeria_staff s
    WHERE s.pizzeria_id = p.id
      AND s.staff_subject = 'staff-a'
  );
