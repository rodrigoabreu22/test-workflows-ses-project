...
export function PizzasPage({ selectedPizzeriaId }: PizzasPageProps) {
  // See PizzeriasPage for the full rationale: `ownerSubject` never reaches
  // the frontend (API hardening), so "is this mine" can only be answered by
  // (1) role — CUSTOMER holds none of the pizza create/update/delete grants
  // in permissions.csv, a guaranteed refusal — or (2) the backend's own 403
  // on a mutation attempt, which we remember per pizza id.
  const identity = getDisplayIdentity()
  const authenticated = identity !== null
  const roles = identity?.roles ?? []
  const isAdmin = roles.includes('ADMIN')
  const isOwner = roles.includes('PIZZERIA_OWNER')
  const isStaff = roles.includes('PIZZERIA_STAFF')
  const canMutate = isAdmin || isOwner || isStaff
            ...
            <form className="form-grid" onSubmit={handleCreate}>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setCreateForm({ ...createForm, ingredients: event.target.value })
                  }
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCreateForm({ ...createForm, pizzeriaId: event.target.value })
                  }
                  required
                >
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setCreateForm({ ...createForm, photoUrl: event.target.value })
                  }
                  ...
                  <tr key={pizza.id}>
                    ...
                    <td className="row-actions">
                      {pizza.canEdit ? (
                        ...
                        <span className="row-actions-empty">—</span>
                      )}
                    </td>
                  </tr>
                ))
